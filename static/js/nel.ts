...
class Player {
    ...
    InitDom() {
        ...
        // [全局定时器] 用于实时监控播放状态
        setInterval(() => {
            let sheetListBox = <HTMLUListElement>document.getElementById("sheetListBox");   // 歌单列表
            ...
            if (this.player.getAttribute("mode") != "fm") {
                // 如果歌单列表存在，更新项目的背景色
                if (sheetListBox) {
                    let sheetListBoxItemSelected = <HTMLLIElement>sheetListBox.children.item(Number(this.player.getAttribute("index")));
                    for (let i = 0; i < sheetListBox.children.length; i++) {
                        let sheetListBoxItem = <HTMLLIElement>sheetListBox.children.item(i);
                        // [其它] 背景
    ...
    attachPlaylist() {
        ...
        if (this.player.getAttribute("mode") != "fm") {
            ...
            for (let i = 0; i < sheetListBox.children.length; i++) {
                ...
                let c = <HTMLLIElement>sheetListBox.children.item(i).cloneNode(true)
