        ...
        this.newCollectDialog = function (id, sheetlist, now, cookie) {
            readFile(path.join(__dirname, "../pages/collect.html"), (err, data) => {
                ...
                collectAddBtn.addEventListener("click", (e) => {
                    ...
                    req.open("GET", `${netease_1.server}/playlist/tracks?op=add&pid=${sheet_select.options[sheet_select.selectedIndex].value}&tracks=${now}&cookie=${cookie}&timestamp=${new Date().getTime()}`);
                    req.send();
                    req.onreadystatechange = function (e) {
                        if (req.status == 200 && req.readyState == 4) {
                            ...
                            if (status == 200) {
                                ...
                                req2.open("GET", `${netease_1.server}/playlist/tracks?op=del&pid=${sheet_select.options[sheet_select.selectedIndex].value}&tracks=${now}&cookie=${cookie}&timestamp=${new Date().getTime()}`);
                                req2.send();
